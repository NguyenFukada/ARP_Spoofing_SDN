...
n_ip = len((ipsrc_csv))    # Get number of different source IPs
print(n_ip)
...
path = HOME + '/Multi-Label-Attack-Detection/ARP_Broadcast/arp_broadcast.csv'
...
if mitm != 0:
    tag_mitm = 'MITM '
if arpflooding !=0:
    tag_arpflooding = 'ARP_FLOOD'
if ddos == 0 and slow_rate == 0 and mitm == 0 and ip_spoofing == 0 and arpflooding == 0:
    tag = 'Normal'
else:
    tag = tag_ddos + tag_slow_rate + tag_mitm + tag_ip_spoofing + tag_arpflooding
...
# features_Mitm_draft = [aps, abps, subARP, miss_match]
# headers1 = ["APS", "ABPS", "SUBARP", "MISS_MAC","ARP_Spoofing"]
...
#     cursor.writerow(features_Mitm_draft)
#     f.close()
